...
uint64_t MyHash(char *plaintext, uint64_t fileLen)
{
    ...
    char *curr = plaintext;
    uint64_t block_len = fileLen / 8 + (fileLen % 8 != 0 ? 1 : 0);
    uint64_t *blocks = new uint64_t[block_len];

    // iterate to populate 64 bit blocks
    for (uint64_t i = 0; i < fileLen / 8; i++)
    {
        blocks[i] = ((uint64_t)curr[0]) << 0 |
                    ((uint64_t)curr[1]) << 8 |
                    ((uint64_t)curr[2]) << 16 |
                    ((uint64_t)curr[3]) << 24 |
                    ((uint64_t)curr[4]) << 32 |
    ...
    // handle any extra bytes with padding
    if (fileLen % 8 != 0)
    {
        int rem = fileLen % 8;
        // clear existing junk values
        blocks[block_len - 1] = 0;
